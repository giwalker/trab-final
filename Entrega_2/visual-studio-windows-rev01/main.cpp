...
int main() {
	...
	while (1) {
		...
		try {
			...
			if (menu_option < 1 || menu_option > 4) throw 0;

			//Get the Id and make the pointer point to the correct list
			if (menu_option != 4) {
				cout << "Insert the target Embbedded System Id: ";
				string aux_target_id;
				target_id = 257;

				while (target_id > 256 || target_id < 0) {
					cin >> aux_target_id;

					if (aux_target_id.length() > 3) cout << "Wrong input, insert the Id again: ";
					else {
						target_id = stoi(aux_target_id);
						if (target_id > 256 || target_id < 0) cout << "Wrong input, insert the Id again: ";
					}
				}
