...
#define BR 9600
	...
	//int h_atual = 0, min_atual = 0, d_atual = 0, m_atual = 0, y_atual = 0;
	//int h_upd = 0, min_upd = 0, d_upd = 0, m_upd = 0, y_upd = 0;
...
//====================================================================//
	class TC_Stm : public Timer, public Temperature_Control, public USART{
			...
			bool FLAG_B_Pressed;
			//int hora_vec[10], minuto_vec[10];
			//int hora, min, dia, mes, ano, count_vec;
			bool FLAG_AC_Block;
			bool FLAG_Smth_Open;
		public:
...
int main(void){
  ...
  while (1){
		EMB.updateRTC();
		//EMB.getTimer(); 				
		p-> setDisplay(p->actual_screen); 	
		p-> updateDisplay();
		p-> getButton();
		EMB.doStuff();
		if (p->getParameter(7) == 1){
			Log_1.sendList();
			p->setParameter(7, 0);		//Zerar P10 - Comunicação Realizada
		}
		if (FLAG_Change_Date){
...
extern "C" void TIM2_IRQHandler(void){		//Routine responsible for the interruption
		...
		//if (!FLAG_Timer_Pause) count_ms++;;		//FLAG_Timer_Pause is responsible for the break in the time counting
...
TC_Stm::TC_Stm(){
	...
	/*
	hora_vec[0] = 8; hora_vec[1] = 10; hora_vec[2] = 12; hora_vec[3] = 13; hora_vec[4] = 15; hora_vec[5] = 16; hora_vec[6] = 18; 
	hora_vec[7] = 21; hora_vec[8] = 22; hora_vec[9] = 23; 
	minuto_vec[0] = 0; minuto_vec[1] = 15; minuto_vec[2] = 12; minuto_vec[3] = 27; minuto_vec[4] = 43; minuto_vec[5] = 5; minuto_vec[6] = 54;
	minuto_vec[7] = 23; minuto_vec[8] = 33; minuto_vec[9] = 37;*/
	/*hora = 0;
	min = 0;
	dia = 9;
	mes = 4;
...
//Send the Linked List via USART and delete the lista
void List::sendList(){
	Node* q = head;
...
//Get the time elapsed and update the value to be displayed

/*void TC_Stm::getTimer(){
		if (!FLAG_OFF_Delay){			//System ON
				total_time_ms = (((getParameter(1))*60000)+((getParameter(2))*1000));
				remaining_time_ms = total_time_ms - count_ms;
				Mt = (remaining_time_ms/60000);
				St = ((remaining_time_ms-(Mt*60000))/1000);
				if (Mt <= 0) Mt = 0;
				if (St <= 0) St = 0;
				Mh = (Mt/10); 
				Ml = (Mt-(Mh*10));
				Sh = (St/10);
				Sl = (St-(Sh*10));
		}		//End if (!FLAG_OFF_Delay)
		
		if (FLAG_OFF_Delay && !FLAG_ON_Delay){		//System Turning OFF
				total_time_ms = getParameter(3)*1000;
				if ((count_ms - initial_time_ms_OFF) >= total_time_ms){
						//l->insertAfterLast(cont_log, 254, 1, h_1, m_1, d_1, me_1, y_1);
						GPIOB->BSRR = (1<<(13+16));  	  //Blue LED OFF
						GPIOB->BSRR = (1<<(9+16));  	  //User LED OFF
						//DESLIGAR AQUI - TIME OUT (1)
						EMB.updateRTC();
						resetTimer();
						Log_1.insertAfterLast(cont_log, 254, 1, h_1, m_1, d_1, me_1, y_1);
						//l->insertAfterLast(cont_log, 254, 1, t->getRTC(1), t->getRTC(2), t->getRTC(3), t->getRTC(4), t->getRTC(5));
						cont_log++;
						pauseTimer();
				}
		}		//End if (FLAG_OFF_Delay)
		
		if (!FLAG_OFF_Delay && FLAG_ON_Delay){		//System Turning ON
				total_time_ms = getParameter(4)*1000;
				if ((total_time_ms - count_ms) <= 0){
						GPIOB->BSRR = (1<<12);   				//Yellow LED ON
						FLAG_ON_Delay = false;
				}
		}

		if (remaining_time_ms <= 0 && !FLAG_OFF_Delay){		
				FLAG_OFF_Delay = true;
				initial_time_ms_OFF = count_ms;
				GPIOB->BSRR = (1<<(12+16));   	//Yellow LED OFF
		}
}		//End void TC_Stm::getTimer()
*/
...
void TC_Stm::getButton(){
		...
		if (GPIOB-> IDR & (1<<7) && !FLAG_B_Pressed) {		//B3 Pressed
			...
			if (actual_screen == 1) {
				//if (!(GPIOB-> IDR & (1<<13)) || !(GPIOB-> IDR & (1<<12)))
				if (FLAG_AC_Block && (GPIOB-> IDR & (1<<13)) && (GPIOB-> IDR & (1<<12))) {
					FLAG_AC_Block = false;
					Log_1.insertAfterLast(p->getId(), cont_log, 2, h_1, m_1, d_1, me_1, y_1);
					cont_log++;
				}
...
/*
//Reset the time counter in the Interruption
void TC_Stm::resetTimer(){
	FLAG_Timer_Pause = false;
	count_ms = 0;
}
//Pause the time counter in the Interruption
void TC_Stm::pauseTimer(){
		FLAG_Timer_Pause = true;
}
*/
